...
import { acceptCaptureJob, CAPTURE_STORAGE_FAILURE_DETAIL, deleteJob, getCapture, getJob, getRun, MAX_CAPTURE_ATTEMPTS, nextCaptureJob, ORPHAN_JOB_RETENTION_MS, putCapture, putJob, putRun } from '../src/db';
...
function run(providerRuns: Run['providerRuns'], status: Run['status'] = 'active'): Run {
  ...
  return { recordVersion: 2, id, browserSessionId: 'current-session', query: 'query', createdAt: Date.now(), windowId: 1, providerRuns, status, slug: 'query', reportFolder, downloadFolder: `deep-research/${reportFolder}` };
}
...
describe('coordinator run guards', () => {
  it('rejects End provider for a finished provider without touching its job or tab', async () => {
    const stored = run({ chatgpt: provider('chatgpt', 3001, 'complete') }, 'complete');
    const id = `${stored.id}:chatgpt`;
    const job: CaptureJob = {
      id, runId: stored.id, provider: 'chatgpt', tabId: 3001, state: 'queued',
      createdAt: Date.now(), attempts: 0, domMarkdown: 'A retained report.', domCitations: [],
    };
    try {
      await putRun(stored);
      await putJob(job);
      await expect(coordinatorTestHooks.handleRequest({
        type: 'provider:end', runId: stored.id, provider: 'chatgpt',
      }, {})).resolves.toMatchObject({ ok: false, code: 'provider_terminal' });
  ...
  it('ends active providers in a mixed run and stops scripts for every provider', async () => {
    ...
    await putRun(stored);
    await expect(coordinatorTestHooks.handleRequest({ type: 'run:end', runId: stored.id }, {}))
  ...
  it('copies a stored capture after its provider was interrupted', async () => {
    const chatgpt = provider('chatgpt', 3004, 'interrupted');
    const stored = run({ chatgpt }, 'complete');
    const id = `${stored.id}:chatgpt`;
    chatgpt.captureId = id;
    await putRun(stored);
    await putCapture(id, {
      rawMarkdown: 'The retained interrupted report.', normalizedMarkdown: 'The retained interrupted report.',
      citations: [], captureMethod: 'dom_only', unplacedCitationCount: 0,
      capturedAt: Date.now(), urlsResolved: 0, urlsUnresolved: 0,
    });
    await expect(coordinatorTestHooks.handleRequest({
      type: 'provider:copy', runId: stored.id, provider: 'chatgpt',
    }, {})).resolves.toMatchObject({ ok: true });
    expect(platform.writeClipboard).toHaveBeenCalledWith(expect.stringContaining('The retained interrupted report.'));
  });
  ...
  it('resumes finalization without redownloading providers already persisted', async () => {
    ...
    chatgpt.saveReceipt = {
      destination: 'downloads', requestedRelativePath: `${stored.downloadFolder}/chatgpt.md`,
  ...
  it('keeps the failure artifact and saves a normal filename for a late capture', async () => {
    ...
    await coordinatorTestHooks.completeProviderCapture(stored.id, 'chatgpt', id);
  ...
  it('preserves a stale in-flight file and commits a second file for the latest capture revision', async () => {
    ...
    const attachment = coordinatorTestHooks.completeProviderCapture(stored.id, 'chatgpt', id);
  ...
  it('processes an unblocked capture independently while another capture save is stalled', async () => {
    ...
    platform.downloadText.mockImplementation(async (path) => {
      if (String(path).includes(later.reportFolder)) await downloadGate;
      return 100;
    });
    try {
      const worker = coordinatorTestHooks.processCaptureQueue();
      await vi.waitFor(() => expect(platform.downloadText).toHaveBeenCalledWith(
        `${later.downloadFolder}/claude.md`, expect.any(String),
      ));
      await expect(coordinatorTestHooks.handleRequest({
        type: 'provider:end', runId: older.id, provider: 'chatgpt',
      }, {})).resolves.toEqual({ ok: true });
      await vi.waitFor(async () => expect(await getJob(olderId)).toBeUndefined());
      ...
      releaseDownload();
      await coordinatorTestHooks.waitForCaptureQueueIdle();
      await deleteJob(olderId);
      await deleteJob(laterId);
    }
  });
  ...
  it('releases capture locks during slow copy work and rejects the superseded lease commit', async () => {
    const stored = run({
      ...
      claude: provider('claude', 944, 'researching'),
    });
    const id = `${stored.id}:chatgpt`;
    const domReport = 'A durable DOM report wins after End supersedes an in-flight clipboard attempt.';
    const staleClipboard = 'A durable DOM report wins after End supersedes an in-flight clipboard attempt with stale clipboard text.';
    let clipboard = 'private clipboard';
    ...
    platform.writeClipboard.mockImplementation(async (text: string) => { clipboard = text; });
    platform.sendTabEvent.mockImplementation(async (_tabId, event) => {
      if ((event as { type?: string }).type === 'capture:copy-now') {
        await copyGate;
    ...
    tabsGet.mockResolvedValue({ id: 943, url: 'https://chatgpt.com/' } as Browser.tabs.Tab);
    try {
      await putRun(stored);
      await putJob({
        id, runId: stored.id, provider: 'chatgpt', tabId: 943,
        state: 'queued', createdAt: Date.now(), attempts: 0,
        domMarkdown: domReport, domCitations: [],
      });

      const drain = coordinatorTestHooks.processCaptureQueue();
      await vi.waitFor(() => expect(platform.sendTabEvent).toHaveBeenCalledWith(
        943, expect.objectContaining({ type: 'capture:copy-now' }),
      ));
      await expect(coordinatorTestHooks.withCaptureAndRunLock(
        stored.id, 'chatgpt', async () => 'lock acquired',
      )).resolves.toBe('lock acquired');
      await expect(coordinatorTestHooks.reconcileRuns()).resolves.toBeUndefined();
      await expect(coordinatorTestHooks.interruptRemovedTab(944)).resolves.toBeUndefined();
      expect((await getRun(stored.id))?.providerRuns.claude?.status).toBe('interrupted');
      expect(browser.runtime.sendMessage).toHaveBeenCalledWith(expect.objectContaining({ type: 'runs:changed' }));
      await expect(coordinatorTestHooks.handleRequest({
        type: 'provider:end', runId: stored.id, provider: 'chatgpt',
      }, {})).resolves.toEqual({ ok: true });
      await vi.waitFor(async () => expect(await getCapture(id)).toMatchObject({
        captureMethod: 'dom_only', rawMarkdown: domReport,
      }));
      expect(await getJob(id)).toBeUndefined();
  ...
  it('returns from Retry after durable scheduling while report preparation is still blocked', async () => {
    const gemini = provider('gemini', 90910, 'failed');
    gemini.statusDetail = CAPTURE_STORAGE_FAILURE_DETAIL;
    gemini.captureRecoveryPending = true;
    const stored = run({ gemini }, 'complete');
    ...
    const fetchSpy = vi.fn(async () => {
      await fetchGate;
      return { url: 'https://canonical.test/retry' };
    });
    vi.stubGlobal('fetch', fetchSpy);
    try {
      await putRun(stored);
      await putJob({
        id, runId: stored.id, provider: 'gemini', tabId: 90910,
        state: 'paused', createdAt: Date.now(), attempts: 3, tabUnavailable: true,
        domMarkdown: 'A retained Grok report remains durable while citation preparation is blocked.',
        domCitations: [{
          url: 'https://vertexaisearch.cloud.google.com/grounding-api-redirect/retry',
          contextBefore: 'citation', contextAfter: 'preparation', domOrder: 0,
        }],
      });

      await expect(coordinatorTestHooks.handleRequest({
        type: 'provider:retry-capture', runId: stored.id, provider: 'gemini',
      }, {})).resolves.toEqual({ ok: true });
  ...
  it('clears unavailable recovery after Retry report and allows saving failed details', async () => {
    const chatgpt = provider('chatgpt', 90911, 'failed');
    ...
    expect((await getRun(stored.id))?.providerRuns.chatgpt?.statusDetail).toContain('No retained report');
    expect(browser.runtime.sendMessage).toHaveBeenCalledWith(expect.objectContaining({ type: 'runs:changed' }));
    await expect(coordinatorTestHooks.handleRequest({
      type: 'provider:save', runId: stored.id, provider: 'chatgpt',
    }, {})).resolves.toMatchObject({ ok: true });
    expect(platform.downloadText).toHaveBeenCalledWith(`${stored.downloadFolder}/FAILED-chatgpt.md`, expect.any(String));
  });

  it('reads an unrecoverable raw job once, deletes it, and clears retry state', async () => {
    const chatgpt = provider('chatgpt', 90914, 'failed');
    chatgpt.captureRecoveryPending = true;
    const stored = run({ chatgpt }, 'complete');
    stored.completedAt = Date.now();
    const id = `${stored.id}:chatgpt`;
    try {
      await putRun(stored);
      await putLegacyJob({
        id, runId: 42 as unknown as string, provider: 'chatgpt', tabId: 90914,
        state: 'paused', createdAt: Date.now(), attempts: 3, domMarkdown: ' ', domCitations: [],
      });
      const originalGet = IDBObjectStore.prototype.get;
      let jobReads = 0;
      const getSpy = vi.spyOn(IDBObjectStore.prototype, 'get').mockImplementation(function (this: IDBObjectStore, key) {
  ...
  it('keeps a malformed retained report and pauses retry for review', async () => {
    const chatgpt = provider('chatgpt', 90916, 'failed');
    chatgpt.captureRecoveryPending = true;
    const stored = run({ chatgpt }, 'complete');
    stored.completedAt = Date.now();
    const id = `${stored.id}:chatgpt`;
    const report = 'A malformed but substantive retained report must remain available for manual review.';
    try {
      await putRun(stored);
      await putLegacyJob({
        id, runId: 42 as unknown as string, provider: 'chatgpt', tabId: 90916,
        state: 'paused', createdAt: Date.now(), attempts: 3, domMarkdown: report, domCitations: [],
      });
      await expect(coordinatorTestHooks.handleRequest({
        type: 'provider:retry-capture', runId: stored.id, provider: 'chatgpt',
      }, {})).resolves.toMatchObject({ ok: false, code: 'capture_review_required' });
      expect((await getRun(stored.id))?.providerRuns.chatgpt).toMatchObject({
        captureRecoveryPending: undefined, captureReviewPending: true,
      });
      const database = await openDB('deep-research-fan-out', 3);
      expect(await database.get('jobs', id)).toMatchObject({ domMarkdown: report });
      database.close();
    } finally { await deleteJob(id); }
  });
  ...
  it('deletes an unusable recovery job and clears stale actions during reconciliation', async () => {
    ...
    try {
      await putRun(stored);
      await putJob({
        id, runId: stored.id, provider: 'chatgpt', tabId: 90915,
  ...
  it('normalizes contradictory recovery flags on a failed provider to Retry only', async () => {
    const chatgpt = provider('chatgpt', 90925, 'failed');
    ...
    try {
      await putRun(stored);
      await putJob({
        id, runId: stored.id, provider: 'chatgpt', tabId: 90925,
        state: 'paused', createdAt: Date.now(), attempts: 3, tabUnavailable: true,
        domMarkdown: 'A valid paused report must expose Retry without stale review actions.', domCitations: [],
      });
  ...
  it('offers malformed retained text for review without failing an active provider', async () => {
    const stored = run({ chatgpt: provider('chatgpt', 90926, 'researching') });
    const id = `${stored.id}:chatgpt`;
    try {
      await putRun(stored);
      await putLegacyJob({
        id, runId: 42 as unknown as string, provider: 'chatgpt', tabId: 90926,
        state: 'paused', createdAt: Date.now(), attempts: 3,
        domMarkdown: 'Malformed retained text remains reviewable while research stays active.', domCitations: [],
      });

      await coordinatorTestHooks.reconcileRuns();

      expect((await getRun(stored.id))?.providerRuns.chatgpt).toMatchObject({
        status: 'researching', captureReviewPending: true, captureRecoveryPending: undefined,
      });
      expect(platform.downloadText).not.toHaveBeenCalledWith(
        `${stored.downloadFolder}/FAILED-chatgpt.md`, expect.any(String),
      );
  ...
  it('offers Retry for an unflagged paused job owned by an already-terminal provider', async () => {
    ...
    try {
      await putRun(stored);
      await putJob({
        id, runId: stored.id, provider: 'chatgpt', tabId: 90927,
        state: 'paused', createdAt: Date.now(), attempts: 3, tabUnavailable: true,
        domMarkdown: 'A terminal provider still owns a usable paused report for manual Retry.', domCitations: [],
      });
  ...
  it('resumes current-session monitoring after a malformed recovery report is discarded', async () => {
    const chatgpt = provider('chatgpt', 90928, 'failed');
    chatgpt.statusDetail = CAPTURE_STORAGE_FAILURE_DETAIL;
    chatgpt.captureRecoveryPending = true;
    chatgpt.captureReviewPending = true;
    const stored = run({ chatgpt }, 'complete');
    ...
    try {
      await putRun(stored);
      await putLegacyJob({
        id, runId: 42 as unknown as string, provider: 'chatgpt', tabId: 90928,
        state: 'paused', createdAt: Date.now(), attempts: 3,
        domMarkdown: 'A malformed retained report can be discarded before monitoring resumes safely.', domCitations: [],
      });
      tabsGet.mockResolvedValue({ id: 90928, url: 'https://chatgpt.com/' } as Browser.tabs.Tab);
      await coordinatorTestHooks.reconcileRuns();
      expect((await getRun(stored.id))?.providerRuns.chatgpt).toMatchObject({
        status: 'failed', captureRecoveryPending: undefined, captureReviewPending: true,
      });
      ...
      expect((await getRun(stored.id))?.providerRuns.chatgpt).toMatchObject({
        status: 'capturing', captureRecoveryPending: undefined, captureReviewPending: undefined,
      });
      expect(platform.sendTabEvent).toHaveBeenCalledWith(90928, expect.objectContaining({
        type: 'content:start', runId: stored.id, resumeOnly: true,
      }));
    } finally { await deleteJob(id); }
  });

  it('uses only read transactions when recovery classification needs no normalization', async () => {
    const chatgpt = provider('chatgpt', 90929, 'failed');
    chatgpt.captureReviewPending = true;
    chatgpt.completedAt = Date.now();
    const stored = run({ chatgpt }, 'complete');
    stored.completedAt = Date.now();
    ...
    try {
      await putRun(stored);
      await putLegacyJob({
        id, runId: 42 as unknown as string, provider: 'chatgpt', tabId: 90929,
        state: 'paused', createdAt: Date.now(), attempts: 3,
        domMarkdown: 'Already-classified malformed retained text needs no further normalization.', domCitations: [],
      });
      const transactionSpy = vi.spyOn(IDBDatabase.prototype, 'transaction');
      try {
        await coordinatorTestHooks.reconcileCaptureRecoveryRecords(stored.id, 'chatgpt');
  ...
  it('continues finalizing other runs when capture queue persistence fails', async () => {
    ...
    const id = `${active.id}:chatgpt`;
    try {
      await putRun(active);
      await putRun(terminal);
      await putJob({
        id, runId: active.id, provider: 'chatgpt', tabId: 90921,
        state: 'queued', createdAt: Date.now(), attempts: 0, tabUnavailable: true,
  ...
  it('surfaces malformed recovery reports for review when Retry is requested', async () => {
    ...
    try {
      ...
      await coordinatorTestHooks.reconcileRuns();
      expect((await getRun(stored.id))?.providerRuns.chatgpt?.captureRecoveryPending).toBeUndefined();
      expect((await getRun(stored.id))?.providerRuns.chatgpt?.captureReviewPending).toBe(true);
      expect(await getRawJob(id)).toBeDefined();
      await expect(coordinatorTestHooks.handleRequest({
        type: 'provider:retry-capture', runId: stored.id, provider: 'chatgpt',
      }, {})).resolves.toMatchObject({ ok: false, code: 'capture_review_required' });
  ...
  it('moves a legacy job with a missing lease into orphan cleanup without losing its report', async () => {
    const runId = `missing-${crypto.randomUUID()}`;
    const id = `${runId}:chatgpt`;
    const report = 'A verified legacy DOM report remains retained while its missing run is recovered.';
    try {
      await putLegacyJob({
        id, runId, provider: 'chatgpt', tabId: 91021, state: 'leased',
        createdAt: Date.now(), attempts: 1, domMarkdown: report, domCitations: [],
      });
      await coordinatorTestHooks.processCaptureQueue();
      const orphan = await getJob(id);
      expect(orphan).toMatchObject({ state: 'orphaned', domMarkdown: report });
      await putJob({ ...orphan!, orphanedAt: Date.now() - ORPHAN_JOB_RETENTION_MS - 1 });
      await coordinatorTestHooks.processCaptureQueue();
      expect(await getJob(id)).toBeUndefined();
    } finally { await deleteJob(id); }
  });
  ...
  it('times out an unresponsive provider copy request and releases the clipboard lock', async () => {
    vi.useFakeTimers();
    const report = 'A sufficiently long provider report proves the clipboard lock was released.';
    let clipboard = 'private clipboard';
    platform.readClipboard.mockImplementation(async () => clipboard);
    platform.writeClipboard.mockImplementation(async (text: string) => { clipboard = text; });
    platform.sendTabEvent.mockImplementation(() => new Promise(() => undefined));
    const job: CaptureJob = {
      id: 'timeout:chatgpt', runId: 'timeout', provider: 'chatgpt', tabId: 301,
      state: 'queued', createdAt: 1, attempts: 0, tabUnavailable: true,
      domMarkdown: report, domCitations: [],
    };

    const timedOut = coordinatorTestHooks.clipboardCapture(platform, job);
    await vi.runAllTimersAsync();
    ...
    const next = coordinatorTestHooks.clipboardCapture(platform, { ...job, id: 'next:chatgpt' });
    await vi.runAllTimersAsync();
    await expect(next).resolves.toMatchObject({ text: report });
  });
  ...
  it('repairs an existing malformed lease atomically while accepting capture', async () => {
    const stored = run({ chatgpt: provider('chatgpt', 9311, 'researching') });
    const id = `${stored.id}:chatgpt`;
    const retained = 'The original complete research report remains durable after a malformed lease is repaired.';
    const legacy = {
      id, runId: stored.id, provider: 'chatgpt', tabId: 9311, state: 'leased',
      createdAt: Date.now(), attempts: 1, domMarkdown: retained, domCitations: [],
      conversationKey: 'https://chatgpt.com/c/retained',
    } as CaptureJob;
    const incoming: CaptureJob = {
      ...legacy, state: 'queued', domMarkdown: 'A later delivery must not replace the retained report.',
    };
    try {
      await putRun(stored);
      await putLegacyJob(legacy);
      await acceptCaptureJob(incoming, (current) => {
        current!.providerRuns.chatgpt!.status = 'capturing';
        return current!;
      });
      expect((await getRun(stored.id))?.providerRuns.chatgpt?.status).toBe('capturing');
      expect(await getJob(id)).toMatchObject({
        state: 'queued', domMarkdown: retained, conversationKey: legacy.conversationKey,
      });
    } finally { await deleteJob(id); }
  });
  ...
  it('replaces an irreparably malformed existing job when it has no retained report', async () => {
    ...
    const id = `${stored.id}:chatgpt`;
    const incoming: CaptureJob = {
      id, runId: stored.id, provider: 'chatgpt', tabId: 9312, state: 'queued',
      createdAt: Date.now(), attempts: 0, domMarkdown: 'A new valid report.', domCitations: [],
    };
    try {
      await putRun(stored);
      await putLegacyJob({ ...incoming, state: 'leased', domMarkdown: 42 as unknown as string });
  ...
  it('returns run_not_found before retained-job review when capture ownership is missing', async () => {
    const runId = crypto.randomUUID();
    const id = `${runId}:chatgpt`;
    try {
      await putLegacyJob({
        id, runId: 42 as unknown as string, provider: 'chatgpt', tabId: 9314,
        state: 'queued', createdAt: Date.now(), attempts: 0,
        domMarkdown: 'A malformed retained report must not mask the missing owning run.', domCitations: [],
      });
      await expect(coordinatorTestHooks.handleRequest({
        type: 'content:capture', runId, provider: 'chatgpt',
        domMarkdown: 'A newly delivered report.', domCitations: [],
      }, contentSender(9314))).resolves.toMatchObject({ ok: false, code: 'run_not_found' });
    } finally { await deleteJob(id); }
  });

  it('returns run_not_found before retained-job review when the provider is missing', async () => {
    const stored = run({ claude: provider('claude', 93141, 'researching') });
    const id = `${stored.id}:chatgpt`;
    try {
      await putRun(stored);
      await putLegacyJob({
        id, runId: 42 as unknown as string, provider: 'chatgpt', tabId: 9314,
        state: 'queued', createdAt: Date.now(), attempts: 0,
        domMarkdown: 'A malformed retained report must not mask the missing provider record.', domCitations: [],
      });
      await expect(coordinatorTestHooks.handleRequest({
        type: 'content:capture', runId: stored.id, provider: 'chatgpt',
        domMarkdown: 'A newly delivered report.', domCitations: [],
  ...
  it('returns run_not_found after atomically checking whether a retained job can be orphaned', async () => {
    const runId = crypto.randomUUID();
    const id = `${runId}:chatgpt`;
    try {
      await putLegacyJob({
  ...
  it('orphans a valid retained job when Retry discovers its run is missing', async () => {
    const runId = crypto.randomUUID();
    const id = `${runId}:chatgpt`;
    try {
      await putJob({
        id, runId, provider: 'chatgpt', tabId: 93143,
        state: 'paused', createdAt: Date.now(), attempts: 3,
        domMarkdown: 'A valid retained report remains recoverable after its owning run disappears.', domCitations: [],
      });
      ...
      expect(await getJob(id)).toMatchObject({
        state: 'orphaned', orphanedAt: expect.any(Number), leasedAt: undefined, leaseToken: undefined,
      });
    } finally { await deleteJob(id); }
  });
  ...
  it('revalidates a stale Discard action and cleans up a newly queued unusable job', async () => {
  ...
  it('revalidates stale Discard state and processes a usable normalized job', async () => {
    const chatgpt = provider('chatgpt', 9316, 'researching');
    chatgpt.captureReviewPending = true;
    const stored = run({ chatgpt });
    const id = `${stored.id}:chatgpt`;
    try {
      await putRun(stored);
      await putJob({
        id, runId: stored.id, provider: 'chatgpt', tabId: 9316,
        state: 'queued', createdAt: Date.now(), attempts: 0,
        domMarkdown: 'A usable normalized report must remain protected from the review discard action.', domCitations: [],
      });
      await expect(coordinatorTestHooks.handleRequest({
        type: 'provider:discard-blocked-job', runId: stored.id, provider: 'chatgpt',
      }, {})).resolves.toEqual({ ok: true });
  ...
  it('serializes Discard behind both capture and run locks', async () => {
    const chatgpt = provider('chatgpt', 9317, 'researching');
    chatgpt.captureReviewPending = true;
    const stored = run({ chatgpt });
    const id = `${stored.id}:chatgpt`;
    ...
    try {
      await putRun(stored);
      await putLegacyJob({
        id, runId: 42 as unknown as string, provider: 'chatgpt', tabId: 9317,
        state: 'queued', createdAt: Date.now(), attempts: 0,
        domMarkdown: 'A blocked report whose review state must not race with concurrent run mutations.', domCitations: [],
      });
      let runHeld = false;
      let captureHeld = false;
      const heldRun = coordinatorTestHooks.withRunLock(stored.id, async () => {
        const concurrent = (await getRun(stored.id))!;
        runHeld = true;
        await runGate;
        concurrent.providerRuns.chatgpt!.statusDetail = 'Concurrent run mutation completed.';
        await putRun(concurrent);
      });
      const heldCapture = coordinatorTestHooks.serializeCapture(id, async () => { captureHeld = true; await captureGate; });
      await vi.waitFor(() => {
        expect(runHeld).toBe(true);
        expect(captureHeld).toBe(true);
      });
      let settled = false;
      const discard = coordinatorTestHooks.handleRequest({
        type: 'provider:discard-blocked-job', runId: stored.id, provider: 'chatgpt',
      ...
      expect(settled).toBe(false);
      const database = await openDB('deep-research-fan-out', 3);
      expect(await database.get('jobs', id)).toBeDefined();
      database.close();
      releaseRun();
      await heldRun;
  ...
  it('finishes a retained report with a missing lease timestamp after its tab closes', async () => {
    const stored = run({ chatgpt: provider('chatgpt', 9121, 'capturing') });
    const id = `${stored.id}:chatgpt`;
    const report = 'A complete original DOM report survives a missing lease timestamp and tab closure.';
    try {
      await putRun(stored);
      await putLegacyJob({
        id, runId: stored.id, provider: 'chatgpt', tabId: 9121,
        state: 'leased', createdAt: Date.now(), attempts: 1,
        domMarkdown: report, domCitations: [],
      });
      await coordinatorTestHooks.interruptRemovedTab(9121);
      await coordinatorTestHooks.waitForCaptureQueueIdle();
      expect((await getRun(stored.id))?.providerRuns.chatgpt).toMatchObject({ status: 'complete', captureId: id });
  ...
  it('never stops a reused prior-session tab when Retry completes', async () => {
    const chatgpt = provider('chatgpt', 9163, 'failed');
    chatgpt.statusDetail = CAPTURE_STORAGE_FAILURE_DETAIL;
    chatgpt.captureRecoveryPending = true;
    const stored = run({ chatgpt }, 'complete');
    stored.browserSessionId = 'prior-session';
    ...
    try {
      await putRun(stored);
      await putJob({
        id, runId: stored.id, provider: 'chatgpt', tabId: 9163,
        state: 'paused', createdAt: Date.now(), attempts: 3, tabUnavailable: true,
        domMarkdown: 'A prior-session paused report can finish without stopping a reused tab ID.', domCitations: [],
      });
      ...
      expect((await getRun(stored.id))?.providerRuns.chatgpt).toMatchObject({ status: 'complete', captureId: id });
      expect(platform.sendTabEvent).not.toHaveBeenCalledWith(9163, { type: 'content:stop', runId: stored.id });
    } finally { await deleteJob(id); }
  });

  it('never stops a reused prior-session tab on direct End', async () => {
    const stored = run({ chatgpt: provider('chatgpt', 9164, 'researching') });
    stored.browserSessionId = 'prior-session';
    await putRun(stored);
    ...
    expect((await getRun(stored.id))?.providerRuns.chatgpt?.status).toBe('abandoned');
    expect(platform.sendTabEvent).not.toHaveBeenCalledWith(9164, { type: 'content:stop', runId: stored.id });
  });
  ...
  it('restores a retained report with a malformed lease after browser restart', async () => {
    const stored = run({ chatgpt: provider('chatgpt', 9161, 'capturing') });
    const id = `${stored.id}:chatgpt`;
    const report = 'A complete DOM report remains available for fallback capture after a browser restart.';
    stored.browserSessionId = 'prior-session';
    try {
      await putRun(stored);
      await putLegacyJob({
        id, runId: stored.id, provider: 'chatgpt', tabId: 9161,
        state: 'leased', createdAt: Date.now(), attempts: 1,
        domMarkdown: report, domCitations: [],
      });
      await coordinatorTestHooks.reconcileRuns();
...
describe('durable acceptance and state recovery regressions', () => {
  ...
  it('rejects a malformed run before committing its capture job', async () => {
    const stored = run({chatgpt:provider('chatgpt',1,'researching')});
    await putRun(stored);
  ...
  it('rejects a malformed capture payload without changing provider state', async () => {
    const stored = run({chatgpt:provider('chatgpt',1,'researching')});
    await putRun(stored);
    const job = jobFor(stored);
    job.domMarkdown = (() => undefined) as unknown as string;
    await expect(acceptCaptureJob(job,(current) => {current!.providerRuns.chatgpt!.status='capturing'; return current!;})).rejects.toThrow('malformed capture job');
    expect(await getJob(job.id)).toBeUndefined();
    expect((await getRun(stored.id))!.providerRuns.chatgpt!.status).toBe('researching');
  });
  it('rejects a cloneable malformed lease before committing either record', async () => {
    const stored = run({chatgpt:provider('chatgpt',1,'researching')});
    await putRun(stored);
    const job = { ...jobFor(stored), state: 'leased', leasedAt: undefined } as CaptureJob;
    await expect(acceptCaptureJob(job,(current) => {
      current!.providerRuns.chatgpt!.status='capturing';
      return current!;
    })).rejects.toThrow('malformed capture job');
    expect(await getJob(job.id)).toBeUndefined();
    expect((await getRun(stored.id))!.providerRuns.chatgpt!.status).toBe('researching');
  });
